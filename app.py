...
def main():
            ...
            if st.button("🔄 Retrain Model", type="primary", use_container_width=True):
                with st.spinner("Retraining model..."):
                    try:
                        ...
                        st.error(f"❌ Retraining failed: {e.stderr}")
                    except Exception as e:
    ...
    with tab2:
        ...
        try:
            ...
            fig = px.line(df, x='Date', y='Accuracy', markers=True, title='Model Accuracy Trend')
            fig.update_layout(yaxis_range=[70, 100], yaxis_title='Accuracy (%)', xaxis_title='')
